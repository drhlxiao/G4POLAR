...
t2polar::t2polar(TTree *t)
{
    ...
    tree->Branch("coincidence",coincidence,"coincidence[1600]/I");
    ...
    tree->Branch("pm3",pm3,Form("pm3[%d]/F",1600));                  ///collected energy deposition, Merlin's simulation results will be used
    ...
    tree->Branch("distance_source_maxbar",&distance_source_maxbar,"distance_source_maxbar/D");   
    ...
    tree->Branch("is_local_max_bar1",is_local_max_bar1,"is_local_max_bar1[npairs]/I");          ///     
    tree->Branch("is_local_max_bar2",is_local_max_bar2,"is_local_max_bar2[npairs]/I");          ///     
...
void t2polar::Init(TTree *t)
{
    ...
    tree->SetBranchAddress("coincidence", coincidence,&b_coincidence);
    ...
    tree->SetBranchAddress("is_local_max_bar1", is_local_max_bar1,&b_is_local_max_bar1);
    tree->SetBranchAddress("is_local_max_bar2", is_local_max_bar2,&b_is_local_max_bar2);
    tree->SetBranchAddress("dist_source", dist_source);
    tree->SetBranchAddress("tot_angle", tot_angle);
...
void t2polar::SetLeavesFrom(t2polar *ti)
{
    ...
    for( int i = 0; i<1600; ++i )
    {
        ...
        pm3[i] =ti->pm3[i]  ;       
        adc[i] =ti->adc[i]  ;       
        //if(coincidence[i]>=0&&coincidence[i]<1600)coincidence[i]=ti->coincidence[i];
    ...
    for(int i=0;i<NPAIRMAX;i++)
    {
      ...
      //  is_local_max_bar1[i]=ti->is_local_max_bar1[i];
      //  is_local_max_bar2[i]=ti->is_local_max_bar2[i];
...
void t2polar::DefaultValues()
{
    ...
    for(int i=0;i<NPAIRMAX;i++)
    {
        ...
        is_local_max_bar1[i]=0;
        is_local_max_bar2[i]=0;
...
void t2polar::GetMaximum(float *data, float &firstmax, int &bar_sec, float &secmax, int &idx2)
{
    double max=0;
    double second_max=0;
...
float t2polar::GetAzimuthalAngle(float *data, double e_thr)
{
    float pmax0,pmax1;
...
void t2polar::FindCoincidencePairs(float threshold,float distance_highlimit,float distance_lowlimit, float source_x, float source_y)
{
	...
	for(int j=i+1;j<nhits;j++)
	{
        ...
        if(dist1>distance_highlimit||dist2>distance_highlimit)continue;
        if(dist1<distance_lowlimit||dist2<distance_lowlimit)continue;
        ...
        Float_t open=v1.Angle(v2)*TMath::RadToDeg(); //between 0-180

        if(open>180-tol)
        {
            mark++;
            pair_mark[i]=pair_mark[i]|(1<<mark);
            pair_mark[j]=pair_mark[j]|(1<<mark);
            open_angle=open;
        }
