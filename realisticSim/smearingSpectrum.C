...
int main(int argc, char *argv[])
{
    if(argc!=4)
    {
        cout<<"smearingSpectrum  response input.root, output.root"<<endl;
        return 0;
    }
    ...
    for(int i=0;i<1600;i++)
    {
        ...
        SmearEnergy(res_low,res_high,hspec[i],hspec_smear[i]);
        SmearEnergy(res_low,res_high,hspec_co[i],hspec_co_smear[i]);
