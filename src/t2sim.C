...
t2sim::t2sim(TTree *tree) 
{
	...
	if (tree == 0) {
		...
		//    tree->Branch("multiple_particles", &multiple_particles, "multiple_particles/o");
...
void t2sim::Init(TTree *tree)
{
	...
	//fChain->SetBranchAddress("multiple_particles", &multiple_particles, &b_multiple_particles);
	Notify();
}
